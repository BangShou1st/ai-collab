...
describe('PlanningView real component workflow', () => {
  it('statusFilterUsesChineseLabels', async () => {
    const wrapper = await mounted()
    expect(wrapper.text()).toContain('正在修复规划问题')
    expect(wrapper.find('[data-value="REPAIRING"]').text()).toBe('正在修复规划问题')
  })
  ...
  it('issueRepairChoosesCorrectMode', async () => {
    mocks.detail.mockResolvedValue(response(detail(plan(), {
      structuredIssues: [issue({ field: 'suggestedAssigneeId', code: 'ASSIGNEE_NOT_PROJECT_MEMBER' })],
    })))
    const wrapper = await mounted()
    await openFirst(wrapper)
    await wrapper.get('.issue-actions button').trigger('click')
    await flushPromises()
    expect(mocks.partialRegenerate).toHaveBeenCalledWith('project-1', 'plan-1',
      expect.objectContaining({ mode: 'REPAIR_ASSIGNMENTS_AND_SOURCES' }))
  })

  it('repairClickCallsPartialRegenerateWithScope', async () => {
    mocks.detail.mockResolvedValue(response(detail(plan(), { structuredIssues: [issue()] })))
    const wrapper = await mounted()
    await openFirst(wrapper)
    await wrapper.get('.issue-actions button').trigger('click')
    ...
    expect(mocks.partialRegenerate).toHaveBeenCalledWith('project-1', 'plan-1', {
      ...
      lockedFields: ['tempKey', 'milestoneTempKey', 'title', 'objective', 'sortOrder'],
      mode: 'REPAIR_DATES_AND_DEPENDENCIES',
    })
  })

  it('historyVersionIsReadOnlyAndCannotConfirm', async () => {
    const wrapper = await mounted()
    await openFirst(wrapper)
  ...
  it('eventTimelineUsesChineseLabels', async () => {
    mocks.events.mockResolvedValue(response([{
      ...
      fromVersionId: 'v1',
      toVersionId: 'v2',
      eventType: 'PLAN_VERSION_RESTORED',
      changedFields: ['startDate'],
      changedTargets: ['t1'],
      issueCodes: ['DEPENDENCY_DATE_CONFLICT'],
      createdAt: '2026-07-28T00:00:00Z',
    }]))
    const wrapper = await mounted()
    await openFirst(wrapper)
    expect(wrapper.text()).toContain('已从历史版本恢复')
    expect(wrapper.text()).toContain('开始日期')
    expect(wrapper.text()).toContain('任务开始时间早于前置任务完成时间')
    expect(wrapper.text()).not.toContain('PLAN_VERSION_RESTORED')
  })
})
