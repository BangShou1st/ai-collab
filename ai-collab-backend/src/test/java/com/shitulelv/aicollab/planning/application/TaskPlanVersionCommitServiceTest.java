...
class TaskPlanVersionCommitServiceTest {
    ...
    @Test
    void commitVersionRecordsSafeDraftFieldsAndTargets() {
        UUID planId = UUID.randomUUID();
        UUID projectId = UUID.randomUUID();
        UUID baseVersionId = UUID.randomUUID();
        UUID newVersionId = UUID.randomUUID();
        UUID actorId = UUID.randomUUID();
        TaskPlanRecord plan = new TaskPlanRecord(
                planId, projectId, "title", "goal", "constraints",
                null, null, 20, "[]", TaskPlanStatus.READY,
                1, baseVersionId, 1L, null, actorId, null, null, null, null);
        PlanMilestone milestone = new PlanMilestone(
                "m1", "Milestone", "Objective", "Description", null, 0, List.of());
        PlanTask originalTask = new PlanTask(
                "t1", "m1", "Task 1", "Objective", "Old description", "MEDIUM",
                BigDecimal.ONE, null, null, null, null, List.of(), List.of(), 0);
        TaskPlanDraft before = new TaskPlanDraft(
                "Summary", List.of("A1"), List.of(),
                List.of(milestone), List.of(originalTask), List.of());
        PlanTask changedTask = new PlanTask(
                "t1", "m1", "Task 1", "Objective", "New description", "MEDIUM",
                BigDecimal.ONE, null, null, null, null, List.of(), List.of(), 0);
        PlanTask addedTask = new PlanTask(
                "t2", "m1", "Task 2", "Objective", "Description", "LOW",
                BigDecimal.ONE, null, null, null, null, List.of("t1"), List.of(), 1);
        TaskPlanDraft after = new TaskPlanDraft(
                "Summary", List.of("A1", "A2"), List.of(),
                List.of(milestone), List.of(changedTask, addedTask), List.of());
        TaskPlanVersionRecord baseRecord = new TaskPlanVersionRecord(
                baseVersionId, planId, 1, "AI_COMPLETE", null, 1L,
                "Summary", "[]", "[]", "[]", "[]", "[]", "{}", actorId, null);
        TaskPlanVersionRecord newRecord = new TaskPlanVersionRecord(
                newVersionId, planId, 2, "MANUAL_EDIT", baseVersionId, 1L,
                "Summary", "[]", "[]", "[]", "[]", "[]", "{}", actorId, null);
        when(repository.requireVersion(projectId, planId, baseVersionId)).thenReturn(baseRecord);
        when(repository.draft(baseRecord)).thenReturn(before);
        when(repository.appendVersion(
                eq(projectId), eq(planId), eq(baseVersionId), eq("MANUAL_EDIT"),
                eq(baseVersionId), eq(after), eq(actorId), any(), eq(TaskPlanStatus.READY)))
                .thenReturn(newVersionId);
        when(repository.requireVersion(projectId, planId, newVersionId)).thenReturn(newRecord);

        service.commitVersion(
                plan, after, TaskPlanVersionSource.MANUAL_EDIT, ValidationAssessment.empty(),
                TaskPlanStatus.READY, "TASK_PLAN_USER_EDITED", actorId, baseVersionId);

        verify(eventRepo).append(argThat(event ->
                event.changedFields().containsAll(List.of(
                        "assumptions", "tasks.t1.description", "tasks.t2"))
                        && event.changedTargets().containsAll(List.of("TASK:t1", "TASK:t2"))
