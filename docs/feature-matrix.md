...
更新日期：2026-08-05。本文档只描述当前代码事实，是判断“已完成、部分完成、未实现”的首要入口；产品目标和后续设计不能替代本矩阵中的实现状态。
...
| 所有权转移 | 完整 | 使用事务、项目/OWNER 行锁和更新行数校验确保原子转移；按旧 OWNER 降级、新 OWNER 升级的顺序满足单 OWNER 约束，并对自转让、目标离组和并发变化返回稳定错误 | `ProjectMemberApplicationService.transferOwnership`、`ProjectController`、`ProjectMembersView.vue` |
...
以下能力当前没有完整实现：自动模型额度检测/切换、跨项目 Agent、开放式多 Agent 编排和不受项目边界约束的通用长期记忆。MCP STDIO 生产传输按安全策略保持禁用。
