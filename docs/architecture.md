...
1. 项目、成员、里程碑、任务、评论和审计；
2. 项目文档上传、解析、向量检索和带引用问答；
3. AI 生成任务规划、人工预览、版本管理和事务确认。

系统当前不包含通用 Agent、通知、甘特图、流式问答或回答反馈。项目类型、四态生命周期、成员退出和所有权转移也尚未实现。
...
Chat、Embedding、Planning 使用独立 OpenAI-compatible 配置：

- `CHAT_*`
- `EMBEDDING_*`
- `PLANNING_*`，未配置项可回退 `CHAT_*`

业务服务依赖统一网关，不依赖某个模型供应商 SDK。切换 Embedding 模型需要重新索引；切换 Chat/Planning 模型不修改既有向量。

这不是通用 Agent。Agent 的状态机、工具策略、审批和预算设计见 `development/agent-design.md`。
